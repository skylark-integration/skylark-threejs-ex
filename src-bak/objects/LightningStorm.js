define([
    ...
    var LightningStorm = function (stormParams) {
        ...
        this.lightningParameters = b.LightningStrike.copyParameters(stormParams.lightningParameters, stormParams.lightningParameters);
        this.lightningParameters.isEternal = false;
        this.lightningMaterial = stormParams.lightningMaterial !== undefined ? stormParams.lightningMaterial : new a.MeshBasicMaterial({ color: 11599871 });
        if (stormParams.onRayPosition !== undefined) {
            this.onRayPosition = stormParams.onRayPosition;
        } else {
            this.onRayPosition = function (source, dest) {
                dest.set((Math.random() - 0.5) * stormParams.size, 0, (Math.random() - 0.5) * stormParams.size);
                var height = a.MathUtils.lerp(stormParams.minHeight, stormParams.maxHeight, Math.random());
        ...
        for (var i = 0; i < this.stormParams.maxLightnings; i++) {
            var lightning = new b.LightningStrike(b.LightningStrike.copyParameters({}, this.lightningParameters));
            var mesh = new a.Mesh(lightning, this.lightningMaterial);
            this.deadLightningsMeshes.push(mesh);
        }
    };
    LightningStorm.prototype = Object.create(a.Object3D.prototype);
    ...
    LightningStorm.prototype.update = function (time) {
        ...
        if (time >= this.nextLightningTime) {
            ...
            if (lightningMesh) {
                var lightningParams1 = b.LightningStrike.copyParameters(lightningMesh.geometry.rayParameters, this.lightningParameters);
                lightningParams1.birthTime = time;
                lightningParams1.deathTime = time + a.MathUtils.lerp(this.stormParams.lightningMinDuration, this.stormParams.lightningMaxDuration, Math.random());
        ...
        while (i < il) {
            ...
            if (prevState === b.LightningStrike.RAY_PROPAGATING && lightning.state > prevState) {
                if (this.onLightningDown) {
                    this.onLightningDown(lightning);
                }
            }
            if (lightning.state === b.LightningStrike.RAY_EXTINGUISHED) {
    ...
    LightningStorm.prototype.getNextLightningTime = function (currentTime) {
        return currentTime + a.MathUtils.lerp(this.stormParams.lightningMinPeriod, this.stormParams.lightningMaxPeriod, Math.random()) / (this.stormParams.maxLightnings + 1);
    };
    LightningStorm.prototype.copy = function (source) {
        a.Object3D.prototype.copy.call(this, source);
        ...
        this.lightningParameters = b.LightningStrike.copyParameters({}, source.lightningParameters);
        this.lightningMaterial = source.stormParams.lightningMaterial;
        this.onLightningDown = source.onLightningDown;
        return this;
    };
    b.LightningStrike.prototype.clone = function () {
