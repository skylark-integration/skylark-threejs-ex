define([
    ...
    function ColorSpaceNode(input, method) {
        b.TempNode.call(this, 'v4');
        this.input = input;
        this.method = method || ColorSpaceNode.LINEAR_TO_LINEAR;
    }
    ColorSpaceNode.Nodes = function () {
        var LinearToLinear = new e.FunctionNode([
            'vec4 LinearToLinear( in vec4 value ) {',
            '\treturn value;',
            '}'
        ].join('\n'));
        var GammaToLinear = new e.FunctionNode([
            'vec4 GammaToLinear( in vec4 value, in float gammaFactor ) {',
            '\treturn vec4( pow( value.xyz, vec3( gammaFactor ) ), value.w );',
            '}'
        ].join('\n'));
        var LinearToGamma = new e.FunctionNode([
            'vec4 LinearToGamma( in vec4 value, in float gammaFactor ) {',
            '\treturn vec4( pow( value.xyz, vec3( 1.0 / gammaFactor ) ), value.w );',
            '}'
        ].join('\n'));
        var sRGBToLinear = new e.FunctionNode([
            'vec4 sRGBToLinear( in vec4 value ) {',
            '\treturn vec4( mix( pow( value.rgb * 0.9478672986 + vec3( 0.0521327014 ), vec3( 2.4 ) ), value.rgb * 0.0773993808, vec3( lessThanEqual( value.rgb, vec3( 0.04045 ) ) ) ), value.w );',
            '}'
        ].join('\n'));
        var LinearTosRGB = new e.FunctionNode([
            'vec4 LinearTosRGB( in vec4 value ) {',
            '\treturn vec4( mix( pow( value.rgb, vec3( 0.41666 ) ) * 1.055 - vec3( 0.055 ), value.rgb * 12.92, vec3( lessThanEqual( value.rgb, vec3( 0.0031308 ) ) ) ), value.w );',
            '}'
        ].join('\n'));
        var RGBEToLinear = new e.FunctionNode([
            'vec4 RGBEToLinear( in vec4 value ) {',
            '\treturn vec4( value.rgb * exp2( value.a * 255.0 - 128.0 ), 1.0 );',
            '}'
        ].join('\n'));
        var LinearToRGBE = new e.FunctionNode([
        ...
        var RGBMToLinear = new e.FunctionNode([
            'vec3 RGBMToLinear( in vec4 value, in float maxRange ) {',
            '\treturn vec4( value.xyz * value.w * maxRange, 1.0 );',
            '}'
        ].join('\n'));
        var LinearToRGBM = new e.FunctionNode([
        ...
        var RGBDToLinear = new e.FunctionNode([
            'vec3 RGBDToLinear( in vec4 value, in float maxRange ) {',
            '\treturn vec4( value.rgb * ( ( maxRange / 255.0 ) / value.a ), 1.0 );',
            '}'
        ].join('\n'));
        var LinearToRGBD = new e.FunctionNode([
        ...
        var cLogLuvM = new c.ConstNode('const mat3 cLogLuvM = mat3( 0.2209, 0.3390, 0.4184, 0.1138, 0.6780, 0.7319, 0.0102, 0.1130, 0.2969 );');
        var LinearToLogLuv = new e.FunctionNode([
        ...
        var cLogLuvInverseM = new c.ConstNode('const mat3 cLogLuvInverseM = mat3( 6.0014, -2.7008, -1.7996, -1.3320, 3.1029, -5.7721, 0.3008, -1.0882, 5.6268 );');
        var LogLuvToLinear = new e.FunctionNode([
    ...
    ColorSpaceNode.getEncodingComponents = function (encoding) {
        ...
        case a.RGBM7Encoding:
            return [
                'RGBM',
                new d.FloatNode(7).setReadonly(true)
            ];
        case a.RGBM16Encoding:
            return [
                'RGBM',
                new d.FloatNode(16).setReadonly(true)
            ];
        case a.RGBDEncoding:
            return [
                'RGBD',
                new d.FloatNode(256).setReadonly(true)
            ];
        case a.GammaEncoding:
            return [
                'Gamma',
                new f.ExpressionNode('float( GAMMA_FACTOR )', 'f')
            ];
        }
    };
    ColorSpaceNode.prototype = Object.create(b.TempNode.prototype);
