define([
    ...
    function FunctionNode(src, includes, extensions, keywords, type) {
        ...
        a.TempNode.call(this, type);
        this.parse(src, includes, extensions, keywords);
    }
    FunctionNode.prototype = Object.create(a.TempNode.prototype);
    ...
    FunctionNode.prototype.generate = function (builder, output) {
        ...
        for (var i = 0; i < matches.length; i++) {
            ...
            if (this.keywords[prop] || this.useKeywords && isGlobal && b.NodeLib.containsKeyword(prop)) {
                var node = this.keywords[prop];
                if (!node) {
                    var keyword = b.NodeLib.getKeywordData(prop);
                    if (keyword.cache)
                        node = builder.keywords[prop];
                    node = node || b.NodeLib.getKeyword(prop, builder);
                    if (keyword.cache)
            ...
            if (this.getIncludeByName(reference) === undefined && b.NodeLib.contains(reference)) {
                builder.include(b.NodeLib.get(reference));
            }
        }
        if (output === 'source') {
            return src;
