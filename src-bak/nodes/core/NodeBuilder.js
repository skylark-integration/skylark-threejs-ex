define([
    ...
    '../misc/TextureCubeNode.js'
], function (a, b, c, d, e, f, g, h, i, j, k, l, m) {
    'use strict';
    var elements = c.NodeUtils.elements, constructors = [
            'float',
            'vec2',
            'vec3',
            'vec4'
    ...
    NodeBuilder.prototype = {
        ...
        include: function (node, parent, source) {
            var includesStruct;
            node = typeof node === 'string' ? d.NodeLib.get(node) : node;
            if (this.context.include === false) {
                return node.name;
            }
            if (node instanceof e.FunctionNode) {
                includesStruct = this.includes.functions;
            } else if (node instanceof f.ConstNode) {
                includesStruct = this.includes.consts;
            } else if (node instanceof g.StructNode) {
            ...
            if (node) {
                ...
                if (node instanceof e.FunctionNode && parent && includes[parent.name] && includes[parent.name].deps.indexOf(node) == -1) {
                ...
                if (nodeCandidate !== undefined) {
                    if (nodeCandidate.isNode) {
                        ...
                        case a.CubeRefractionMapping:
                            return new l.CubeTextureNode(nodeCandidate);
                            break;
                        case a.CubeUVReflectionMapping:
                        case a.CubeUVRefractionMapping:
                            return new m.TextureCubeNode(new k.TextureNode(nodeCandidate));
                            break;
                        default:
                            return new k.TextureNode(nodeCandidate);
                        }
                    } else if (nodeCandidate.isVector2) {
                        return new h.Vector2Node(nodeCandidate);
                    } else if (nodeCandidate.isVector3) {
                        return new i.Vector3Node(nodeCandidate);
                    } else if (nodeCandidate.isVector4) {
                        return new j.Vector4Node(nodeCandidate);
                    }
                }
            }
        },
        ...
        getTextureEncodingFromMap: function (map) {
            ...
            if (encoding === a.LinearEncoding && this.context.gamma) {
                encoding = a.GammaEncoding;
            }
            return encoding;
        }
    };
