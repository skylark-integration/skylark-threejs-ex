define([
    ...
    var MD2CharacterComplex = function () {
        ...
        this.loadParts = function (config) {
            ...
            var loader = new b.MD2Loader();
            loader.load(config.baseUrl + config.body, function (geo) {
                var boundingBox = new a.Box3();
        ...
        this.updateMovementModel = function (delta) {
            ...
            if (controls.moveForward)
                this.speed = a.MathUtils.clamp(this.speed + delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
            if (controls.moveBackward)
                this.speed = a.MathUtils.clamp(this.speed - delta * this.backAcceleration, this.maxReverseSpeed, this.maxSpeed);
            var dir = 1;
            if (controls.moveLeft) {
                this.bodyOrientation += delta * this.angularSpeed;
                this.speed = a.MathUtils.clamp(this.speed + dir * delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
            }
            ...
            if (!(controls.moveForward || controls.moveBackward)) {
                if (this.speed > 0) {
                    ...
                    this.speed = a.MathUtils.clamp(this.speed - k * delta * this.frontDecceleration, 0, this.maxSpeed);
                } else {
                    var k = exponentialEaseOut(this.speed / this.maxReverseSpeed);
                    this.speed = a.MathUtils.clamp(this.speed + k * delta * this.backAcceleration, this.maxReverseSpeed, 0);
                }
            }
        ...
        function loadTextures(baseUrl, textureUrls) {
            var textureLoader = new a.TextureLoader();
            var textures = [];
            for (var i = 0; i < textureUrls.length; i++) {
                textures[i] = textureLoader.load(baseUrl + textureUrls[i], checkLoadingComplete);
                textures[i].mapping = a.UVMapping;
                textures[i].name = textureUrls[i];
                textures[i].encoding = a.sRGBEncoding;
            }
            return textures;
        }
        function createPart(geometry, skinMap) {
            var materialWireframe = new a.MeshLambertMaterial({
                color: 16755200,
                wireframe: true,
                morphTargets: true,
                morphNormals: true
            });
            var materialTexture = new a.MeshLambertMaterial({
                color: 16777215,
                wireframe: false,
                map: skinMap,
                morphTargets: true,
            ...
            var mesh = new c.MorphBlendMesh(geometry, materialTexture);
