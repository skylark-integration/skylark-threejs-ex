define(['skylark-threejs'], function (a) {
    ...
    MeshReceiver.prototype = {
        ...
        buildMeshes: function (meshPayload) {
            ...
            let bufferGeometry = new a.BufferGeometry();
            if (buffers.vertices !== undefined && buffers.vertices !== null) {
                bufferGeometry.setAttribute('position', new a.BufferAttribute(new Float32Array(buffers.vertices), 3));
            }
            if (buffers.indices !== undefined && buffers.indices !== null) {
                bufferGeometry.setIndex(new a.BufferAttribute(new Uint32Array(buffers.indices), 1));
            }
            if (buffers.colors !== undefined && buffers.colors !== null) {
                bufferGeometry.setAttribute('color', new a.BufferAttribute(new Float32Array(buffers.colors), 3));
            }
            if (buffers.normals !== undefined && buffers.normals !== null) {
                bufferGeometry.setAttribute('normal', new a.BufferAttribute(new Float32Array(buffers.normals), 3));
            } else {
                bufferGeometry.computeVertexNormals();
            }
            if (buffers.uvs !== undefined && buffers.uvs !== null) {
                bufferGeometry.setAttribute('uv', new a.BufferAttribute(new Float32Array(buffers.uvs), 2));
            }
            if (buffers.skinIndex !== undefined && buffers.skinIndex !== null) {
                bufferGeometry.setAttribute('skinIndex', new a.BufferAttribute(new Uint16Array(buffers.skinIndex), 4));
            }
            if (buffers.skinWeight !== undefined && buffers.skinWeight !== null) {
                bufferGeometry.setAttribute('skinWeight', new a.BufferAttribute(new Float32Array(buffers.skinWeight), 4));
            }
            ...
            if (useOrgMesh) {
                ...
                if (geometryType === 0) {
                    mesh = new a.Mesh(bufferGeometry, material);
                } else if (geometryType === 1) {
                    mesh = new a.LineSegments(bufferGeometry, material);
                } else {
                    mesh = new a.Points(bufferGeometry, material);
                }
                mesh.name = meshName;
                meshes.push(mesh);
            }
