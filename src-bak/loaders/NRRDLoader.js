define([
    ...
    NRRDLoader.prototype = Object.assign(Object.create(a.Loader.prototype), {
        ...
        parse: function (data) {
            ...
            if (headerObject.encoding === 'gzip' || headerObject.encoding === 'gz') {
                var inflate = new b.Zlib.Gunzip(new Uint8Array(_data));
            ...
            var spacingX = new a.Vector3(headerObject.vectors[0][0], headerObject.vectors[0][1], headerObject.vectors[0][2]).length();
            var spacingY = new a.Vector3(headerObject.vectors[1][0], headerObject.vectors[1][1], headerObject.vectors[1][2]).length();
            var spacingZ = new a.Vector3(headerObject.vectors[2][0], headerObject.vectors[2][1], headerObject.vectors[2][2]).length();
            volume.spacing = [
                spacingX,
                spacingY,
                spacingZ
            ];
            volume.matrix = new a.Matrix4();
            var _spaceX = 1;
            var _spaceY = 1;
            var _spaceZ = 1;
            ...
            volume.inverseMatrix = new a.Matrix4();
            volume.inverseMatrix.getInverse(volume.matrix);
            volume.RASDimensions = new a.Vector3(volume.xLength, volume.yLength, volume.zLength).applyMatrix4(volume.matrix).round().toArray().map(Math.abs);
