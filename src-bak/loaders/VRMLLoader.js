define([
    "skylark-threejs",
    '../libs/chevrotain.module.min.js'
], function (a, b) {
    'use strict';
    var VRMLLoader = function () {
        if (typeof b.chevrotain === 'undefined') {
        ...
        VRMLLoader.prototype = Object.assign(Object.create(a.Loader.prototype), {
            constructor: VRMLLoader,
            load: function (url, onLoad, onProgress, onError) {
                var scope = this;
                var path = scope.path === '' ? a.LoaderUtils.extractUrlBase(url) : scope.path;
            ...
            parse: function (data, path) {
                ...
                function createTokens() {
                    var createToken = b.chevrotain.createToken;
                    ...
                    var Comment = createToken({
                        name: 'Comment',
                        pattern: /#.*/,
                        group: b.chevrotain.Lexer.SKIPPED
                    });
                ...
                function parseTree(tree) {
                    ...
                    for (var i = 0, l = nodes.length; i < l; i++) {
                        ...
                        if (object instanceof a.Object3D)
                            scene.add(object);
                    }
                    return scene;
                }
                ...
                function buildGroupingNode(node) {
                    ...
                    for (var i = 0, l = fields.length; i < l; i++) {
                        ...
                        case 'children':
                            ...
                            break;
                        case 'rotation':
                            var axis = new a.Vector3(fieldValues[0], fieldValues[1], fieldValues[2]);
                ...
                function buildBackgroundNode(node) {
                    ...
                    var radius = 10000;
                    if (skyColor) {
                        var skyGeometry = new a.SphereBufferGeometry(radius, 32, 16);
                        var skyMaterial = new a.MeshBasicMaterial({
                            fog: false,
                            side: a.BackSide,
                            depthWrite: false,
                            depthTest: false
                        ...
                        var sky = new a.Mesh(skyGeometry, skyMaterial);
                        group.add(sky);
                    }
                    if (groundColor) {
                        if (groundColor.length > 0) {
                            var groundGeometry = new a.SphereBufferGeometry(radius, 32, 16, 0, 2 * Math.PI, 0.5 * Math.PI, 1.5 * Math.PI);
                            var groundMaterial = new a.MeshBasicMaterial({
                                fog: false,
                                side: a.BackSide,
                                vertexColors: true,
                                depthWrite: false,
                            ...
                            var ground = new a.Mesh(groundGeometry, groundMaterial);
                            group.add(ground);
                        }
                    }
                ...
                function buildShapeNode(node) {
                    var fields = node.fields;
                    var material = new a.MeshBasicMaterial({ color: 0 });
                    var geometry;
                    for (var i = 0, l = fields.length; i < l; i++) {
                        var field = fields[i];
                        var fieldName = field.name;
                    ...
                    if (geometry && geometry.attributes.position) {
                        ...
                        if (type === 'points') {
                            var pointsMaterial = new a.PointsMaterial({ color: 16777215 });
                            ...
                            object = new a.Points(geometry, pointsMaterial);
                        } else if (type === 'line') {
                            var lineMaterial = new a.LineBasicMaterial({ color: 16777215 });
                            if (geometry.attributes.color !== undefined) {
                            ...
                            object = new a.LineSegments(geometry, lineMaterial);
                        } else {
                            if (geometry._solid !== undefined) {
                                material.side = geometry._solid ? a.FrontSide : a.DoubleSide;
                            }
                            if (geometry.attributes.color !== undefined) {
                                material.vertexColors = true;
                            }
                            object = new a.Mesh(geometry, material);
                        }
                    } else {
                        object = new a.Object3D();
                        object.visible = false;
                    }
                    return object;
                }
                function buildAppearanceNode(node) {
                    var material = new a.MeshPhongMaterial();
                    var transformData;
                    var fields = node.fields;
                    for (var i = 0, l = fields.length; i < l; i++) {
                        var field = fields[i];
                        ...
                        case 'material':
                            if (fieldValues[0] !== null) {
                                ...
                                material = new a.MeshBasicMaterial({ color: 0 });
                            }
                            break;
                        case 'texture':
                ...
                function buildMaterialNode(node) {
                    ...
                    for (var i = 0, l = fields.length; i < l; i++) {
                        ...
                        case 'ambientIntensity':
                            break;
                        case 'diffuseColor':
                            materialData.diffuseColor = new a.Color(fieldValues[0], fieldValues[1], fieldValues[2]);
                            break;
                        case 'emissiveColor':
                            materialData.emissiveColor = new a.Color(fieldValues[0], fieldValues[1], fieldValues[2]);
                            break;
                        case 'shininess':
                ...
                function buildPixelTextureNode(node) {
                    var texture;
                    var wrapS = a.RepeatWrapping;
                    var wrapT = a.RepeatWrapping;
                    var fields = node.fields;
                    for (var i = 0, l = fields.length; i < l; i++) {
                        var field = fields[i];
                        var fieldName = field.name;
                        ...
                        case 'image':
                            ...
                            texture = new a.DataTexture(data, width, height, useAlpha === true ? a.RGBAFormat : a.RGBFormat);
                ...
                function buildTextureTransformNode(node) {
                    var transformData = {
                        center: new a.Vector2(),
                        rotation: new a.Vector2(),
                        scale: new a.Vector2(),
                        translation: new a.Vector2()
                    };
                    var fields = node.fields;
                    for (var i = 0, l = fields.length; i < l; i++) {
                        var field = fields[i];
                ...
                function buildIndexedFaceSetNode(node) {
                    ...
                    if (color) {
                        if (colorPerVertex === true) {
                            if (colorIndex && colorIndex.length > 0) {
                                ...
                                colorAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new a.Float32BufferAttribute(color, 3));
                            }
                        } else {
                            if (colorIndex && colorIndex.length > 0) {
                    ...
                    if (normal) {
                        if (normalPerVertex === true) {
                            if (normalIndex && normalIndex.length > 0) {
                                ...
                                normalAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new a.Float32BufferAttribute(normal, 3));
                            }
                        } else {
                            if (normalIndex && normalIndex.length > 0) {
                    ...
                    if (texCoord) {
                        if (texCoordIndex && texCoordIndex.length > 0) {
                            ...
                            uvAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new a.Float32BufferAttribute(texCoord, 2));
                        }
                    }
                    var geometry = new a.BufferGeometry();
                    positionAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new a.Float32BufferAttribute(coord, 3));
                ...
                function buildIndexedLineSetNode(node) {
                    ...
                    if (color) {
                        if (colorPerVertex === true) {
                            if (colorIndex.length > 0) {
                                ...
                                colorAttribute = toNonIndexedAttribute(expandedLineIndex, new a.Float32BufferAttribute(color, 3));
                            }
                        } else {
                            if (colorIndex.length > 0) {
                    ...
                    var geometry = new a.BufferGeometry();
                    var positionAttribute = toNonIndexedAttribute(expandedLineIndex, new a.Float32BufferAttribute(coord, 3));
                    geometry.setAttribute('position', positionAttribute);
                    if (colorAttribute)
                        geometry.setAttribute('color', colorAttribute);
                    geometry._type = 'line';
                ...
                function buildPointSetNode(node) {
                    ...
                    var geometry = new a.BufferGeometry();
                    geometry.setAttribute('position', new a.Float32BufferAttribute(coord, 3));
                    if (color)
                        geometry.setAttribute('color', new a.Float32BufferAttribute(color, 3));
                    geometry._type = 'points';
                    return geometry;
                }
                function buildBoxNode(node) {
                    var size = new a.Vector3(2, 2, 2);
                    var fields = node.fields;
                    for (var i = 0, l = fields.length; i < l; i++) {
                        var field = fields[i];
                        var fieldName = field.name;
                    ...
                    var geometry = new a.BoxBufferGeometry(size.x, size.y, size.z);
                    return geometry;
                }
                function buildConeNode(node) {
                    ...
                    var geometry = new a.ConeBufferGeometry(radius, height, 16, 1, openEnded);
                    return geometry;
                }
                function buildCylinderNode(node) {
                    ...
                    var geometry = new a.CylinderBufferGeometry(radius, radius, height, 16, 1);
                    return geometry;
                }
                function buildSphereNode(node) {
                    ...
                    var geometry = new a.SphereBufferGeometry(radius, 16, 16);
                    return geometry;
                }
                ...
                var vA = new a.Vector3();
                var vB = new a.Vector3();
                var vC = new a.Vector3();
                var uvA = new a.Vector2();
                var uvB = new a.Vector2();
                var uvC = new a.Vector2();
                function computeAttributeFromIndexedData(coordIndex, index, data, itemSize) {
                    ...
                    return new a.Float32BufferAttribute(array, itemSize);
                }
                function computeAttributeFromFaceData(index, faceData) {
                    ...
                    return new a.Float32BufferAttribute(array, 3);
                }
                ...
                function toNonIndexedAttribute(indices, attribute) {
                    ...
                    return new a.Float32BufferAttribute(array2, itemSize);
                }
                var ab = new a.Vector3();
                var cb = new a.Vector3();
                function computeNormalAttribute(index, coord, creaseAngle) {
                    ...
                    return new a.Float32BufferAttribute(normals, 3);
                }
                ...
                function toColorArray(colors) {
                    ...
                    for (var i = 0, l = colors.length; i < l; i += 3) {
                        array.push(new a.Color(colors[i], colors[i + 1], colors[i + 2]));
                    }
                    return array;
                }
                function paintFaces(geometry, radius, angles, colors, topDown) {
                    ...
                    for (var i = 0, l = colors.length; i < l; i++) {
                        ...
                        var point = new a.Vector3();
                        point.setFromSphericalCoords(radius, angle, 0);
                        thresholds.push(point);
                    }
                    var indices = geometry.index;
                    var positionAttribute = geometry.attributes.position;
                    var colorAttribute = new a.BufferAttribute(new Float32Array(geometry.attributes.position.count * 3), 3);
                    var position = new a.Vector3();
                    var color = new a.Color();
                ...
                var textureLoader = new a.TextureLoader(this.manager);
        ...
        function VRMLLexer(tokens) {
            this.lexer = new b.chevrotain.Lexer(tokens);
        }
        VRMLLexer.prototype = {
        ...
        function VRMLParser(tokenVocabulary) {
            b.chevrotain.Parser.call(this, tokenVocabulary);
            var $ = this;
        ...
        VRMLParser.prototype = Object.create(b.chevrotain.Parser.prototype);
