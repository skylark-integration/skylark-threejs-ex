define(["skylark-threejs"], function (a) {
    ...
    BasisTextureLoader.prototype = Object.assign(Object.create(a.Loader.prototype), {
        ...
        _createTexture: function (buffer) {
            ...
            var texturePending = this._allocateWorker(taskCost).then(_worker => {
                ...
                case BasisTextureLoader.BASIS_FORMAT.cTFASTC_4x4:
                    texture = new a.CompressedTexture(mipmaps, width, height, a.RGBA_ASTC_4x4_Format);
                    break;
                case BasisTextureLoader.BASIS_FORMAT.cTFBC7_M5:
                    texture = new a.CompressedTexture(mipmaps, width, height, a.RGBA_BPTC_Format);
                    break;
                case BasisTextureLoader.BASIS_FORMAT.cTFBC1:
                case BasisTextureLoader.BASIS_FORMAT.cTFBC3:
                    texture = new a.CompressedTexture(mipmaps, width, height, BasisTextureLoader.DXT_FORMAT_MAP[config.format], a.UnsignedByteType);
                    break;
                case BasisTextureLoader.BASIS_FORMAT.cTFETC1:
                    texture = new a.CompressedTexture(mipmaps, width, height, a.RGB_ETC1_Format);
                    break;
                case BasisTextureLoader.BASIS_FORMAT.cTFPVRTC1_4_RGB:
                    texture = new a.CompressedTexture(mipmaps, width, height, a.RGB_PVRTC_4BPPV1_Format);
                    break;
                case BasisTextureLoader.BASIS_FORMAT.cTFPVRTC1_4_RGBA:
                    texture = new a.CompressedTexture(mipmaps, width, height, a.RGBA_PVRTC_4BPPV1_Format);
                    break;
                default:
                    throw new Error('THREE.BasisTextureLoader: No supported format available.');
                }
                texture.minFilter = mipmaps.length === 1 ? a.LinearFilter : a.LinearMipmapLinearFilter;
        ...
        _initTranscoder: function () {
            if (!this.transcoderPending) {
                var jsLoader = new a.FileLoader(this.manager);
                jsLoader.setPath(this.transcoderPath);
                var jsContent = new Promise((resolve, reject) => {
                    jsLoader.load('basis_transcoder.js', resolve, undefined, reject);
                });
                var binaryLoader = new a.FileLoader(this.manager);
