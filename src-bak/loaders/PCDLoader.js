define(["skylark-threejs"], function (a) {
    ...
    PCDLoader.prototype = Object.assign(Object.create(a.Loader.prototype), {
        ...
        parse: function (data, url) {
            ...
            var textData = a.LoaderUtils.decodeText(new Uint8Array(data));
            ...
            if (position.length > 0)
                geometry.setAttribute('position', new a.Float32BufferAttribute(position, 3));
            if (normal.length > 0)
                geometry.setAttribute('normal', new a.Float32BufferAttribute(normal, 3));
            if (color.length > 0)
                geometry.setAttribute('color', new a.Float32BufferAttribute(color, 3));
            geometry.computeBoundingSphere();
            var material = new a.PointsMaterial({ size: 0.005 });
            if (color.length > 0) {
                material.vertexColors = true;
            } else {
                material.color.setHex(Math.random() * 16777215);
            }
            var mesh = new a.Points(geometry, material);
